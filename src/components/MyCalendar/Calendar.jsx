...
const Calendar = () => {
  ...
  const [editingEvent, setEditingEvent] = useState(null); // FIXED: Initialize editing state
  ...
  const handleClickedEvent = (event, eventIndex) => {
    ...
    setSelectedTag(event.tag || defaultTags[0]); // FIXED: Load tag when editing
  ...
  for (let i = 1; i <= endOfMonth.date(); i++) days.push(i);
  for (let i = endDay; i < 6; i++) days.push(null);
            ...
            >
              ...
              {events[dateKey] &&
                events[dateKey].map((event, eventIndex) => (
                  ...
                  >
                    <span className="px-3 py-1 rounded text-white" style={{ backgroundColor: event.tag?.color }}>
                      {event.tag?.name}
                    </span>
                    <span>
